...
module('Integration | Component | star-rating', function(hooks) {
  ...
  test('The setRating action', async function(assert) {
    this.set('movie', EmberObject.create({ rating: 3 }));
    this.set('actions', {
    ...
    await render(hbs`{{star-rating rating=rating onClick=(action 'updateRating' movie)}}`);
    await click('[data-test-rr=star-rating-5]');
    assert.equal(this.get('movie.rating'), 5, "The clicked star's rating is correctly sent");
  });
});
